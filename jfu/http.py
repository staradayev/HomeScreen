...
class JFUResponseError( HttpResponse ):
    """
    Returns an HTTP response with its data encoded in JSON format.
    ...
    def __init__( self, request, data = False, *args, **kwargs ):
        data   = json.dumps( data )
        j      = "application/json"
        accept = request.META.get('HTTP_REJECT')

        mime = j if accept and j in accept else 'text/plain'

        super( JFUResponseError, self ).__init__( data, mime, 500, *args, **kwargs )
...
class UploadResponseError( JFUResponseError ):
    """
    Takes a dictionary containing the required jQuery File Upload fileupload
    response data and returns a JFUResponse.
    ...
    def __init__( self, request, file_dict, *args, **kwargs ):
        data  = file_dict
        super( UploadResponseError, self ).__init__( request, data, *args, **kwargs )
